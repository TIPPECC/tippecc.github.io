	...
	$: {
		filledSections = activeRanges.map((range) => {
			...
			if (
				...
				startRelative < fixedBeginYear ||
				endRelative > fixedEndYear
			) {
...
<div class="timeline-container">
	<div class="timeline">
		...
		{#each filledSections as section}
			<div class="filled-section" style="left: {section.startPosition}%; width: {section.width}%">
				<span class="filled-section-label" style="margin-top: -8px;"
					>{section.startYear}-{section.endYear}</span
				>
			</div>
		{/each}
	</div>
